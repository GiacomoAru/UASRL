            ...
            for _ in range(args.update_frequency):
                ...
                if global_step > args.learning_starts:
                    ...
                    if global_step % args.loss_log_interval == 0:
                        ...
                        # Richiami la funzione specificando cosa vuoi (es. solo media, o media e massimo)
                        training_stats_divided = extract_and_reset_stats(training_stats, aggregations=['mean'])
                        
                        # Aggiungi le metriche calcolate esternamente (come il tempo/SPS)
                        current_time = time.time()
                        if 'time' not in training_stats_divided:
                            training_stats_divided['time'] = {}
...
while run_id < 10:
    ...
    args.policy_lr = 0.0007465702385030503
    args.q_lr = 0.0007465702385030503
    args.alpha_lr = 0.0007465702385030503
    
    args.tau = 0.007785416182435299
    args.policy_frequency = 1
    args.target_entropy = -3.6316437534168946
    args.batch_size = 128
    args.gamma = 0.9865955384955476
