...
try:
    ...
    while global_step < args.total_timesteps:
        ...
        if episodic_stats != {} and episodic_stats["reward"] > best_reward:
            ...
            print(f"[{global_step}/{args.total_timesteps}] Models saved, suffix: _c{curriculum_step}_best")
        ...
        if env_step > args.learning_starts:
            ...
            for _ in range(args.update_frequency):
                ...
                for i, q in enumerate(qf_ensemble):
                    indices = torch.randint(0, args.batch_size, (args.batch_size,), device=data.actions.device)
                    
                    current_qf_val = q(data.observations[indices], data.actions[indices]).reshape(-1)
